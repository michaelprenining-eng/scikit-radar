# TODO: remove simulation, process all frames for movement analysis
...
radar_measured = skradar.FMCWRadar(
    B=B_samp,
    fc=fc,
    N_f=N_f,
    T_f=1 / fs_f,
...
scene = skradar.Scene([radar_measured, radar_measured], [calib_target])

results = np.empty([all_data.shape[0],tx_pos.shape[1],rx_pos.shape[1], nr_of_targets], dtype=int)
for frame_idx in range(all_data.shape[0]):
    # store measurement data
    if data.ndim == 3:  # add axis that scikit processing can be used
        radar_measured.s_if = all_data[frame_idx,None,:,:,:]
        radar_measured.s_if_noisy = all_data[frame_idx,None,:,:,:]
    else:
    ...
    target_dists = radar_measured.ranges / 2  # halve values to account for round-trip ranges
    resolution = target_dists[1] * zp_fact_range
    print(f"Resolution = {resolution}")
    target_dists_plot = target_dists[: len(radar_measured.ranges) // 2]
    if axis_frequency:
    ...
    peak_pos_idx = np.argmax(rp_measured_noisy_abs_chirp_mean[:,:,calib_peak_search_idx],axis=2) + calib_peak_search_idx[0]
    print(f"Calib target pos = {np.round(target_dists[peak_pos_idx], 2)}m")

    rp_measured_noisy_plot_dB = 20 * np.log10(rp_measured_noisy_abs_chirp_mean)[:,:,: len(radar_measured.ranges) // 2]
    rp_measured_plot_dB = 20 * np.log10(rp_measured_abs_chirp_mean)[:,:,: len(radar_measured.ranges) // 2]

    # fig_rp, (ax_rp_measured, ax_rp_noisy_measured) = plt.subplots(2,1,num="range_profiles",figsize=[8,6],layout="compressed")
    for tx_idx in range(radar_measured.rp.shape[0]):
        for rx_idx in range(radar_measured.rp.shape[1]):
            ...
            peak_indices_measured, peak_dict_measured = scipy.signal.find_peaks(rp_measured_plot_dB[tx_idx, rx_idx],height=(None, None))
            peak_heights_measured = peak_dict_measured['peak_heights'][peak_indices_measured>=min_peak_search_idx]
            peak_indices_measured = peak_indices_measured[peak_indices_measured>=min_peak_search_idx]
            highest_peak_indices_measured = peak_indices_measured[np.argsort(peak_heights_measured)[-nr_of_targets:]]
            results[frame_idx,tx_idx,rx_idx] = highest_peak_indices_measured
            # ax_rp_measured.plot(
            #     target_dists_plot, rp_measured_plot_dB[tx_idx,rx_idx] - np.max(rp_measured_plot_dB), label=f"tx{tx_idx}, rx{rx_idx}"
            # )
            # ax_rp_measured.plot(
            #     target_dists_plot[highest_peak_indices_measured], rp_measured_plot_dB[tx_idx,rx_idx][highest_peak_indices_measured] - np.max(rp_measured_plot_dB), "x"
            # )

np.save("error", np.append(target_dists_plot, rp_measured_noisy_plot_dB))

plt.figure()
plt.plot(target_dists_plot[results[:,0,0,0]])
plt.plot(target_dists_plot[results[:,0,0,1]])
plt.show()

# ax_rp_measured.legend()
# ax_rp_measured.grid(True)
# ax_rp_measured.set_title("Measurement")
# ax_rp_measured.set_ylabel("Normalized Power (dB)")
# ax_rp_measured.set_xlim([0,target_dists_plot[-1]/range_zoom])
# ax_rp_noisy_measured.legend()
# ax_rp_noisy_measured.grid(True)
# ax_rp_noisy_measured.set_title("Added Errors")
# ax_rp_noisy_measured.set_ylabel("Normalized Power (dB)")
# ax_rp_noisy_measured.set_xlim([0,target_dists_plot[-1]/range_zoom])

# if axis_frequency:
#     ax_rp_measured.set_xlabel("Frequency (MHz)")
#     ax_rp_noisy_measured.set_xlabel("Frequency (MHz)")
# else:
#     ax_rp_measured.set_xlabel("Range (m)")
